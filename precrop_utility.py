...
for split in ["train", "val", "test"]:
    ...
    for d in disaster_dirs:
        ...
        image_post_files.append(
            natsorted(
                [
                    os.path.join(disaster_path, "images", f)
        ...
        label_post_files.append(
            natsorted(
                [
                    os.path.join(disaster_path, "labels", f)
            ...
            for feature in features:
                if "wkt" in feature and "properties" in feature:
                    ...
                    if props.get("feature_type") == "building":
                        ...
                        if (
                            damage_type in damage_classes
                            or damage_type == -1
                            or damage_type == "un-classified"
                        ):
                            ...
                            if image_post is None:
                                ...
                                image_post = np.clip(image_post, 0, 255).astype(
                                    np.uint8
                                )
                            ...
                            if isinstance(geom, Polygon) and geom.is_valid:
                                ...
                                if patch.shape[0] > 0 and patch.shape[1] > 0:
                                    ...
                                    patch = np.zeros(
                                        (PATCH_SIZE, PATCH_SIZE, image_post.shape[-1]),
